...
{{/AUTH}}
        ...
        return div(
                nav(
                        h1("{{APP_NAME_CAMEL}}"),
                        div(
                                button("Home").onClick(e -> currentPage.set("home")).build(),
                                button("About").onClick(e -> currentPage.set("about")).build()
{{#AUTH}}
                                , loggedIn.get() != null && loggedIn.getBool()
                                        ? button("Logout").onClick(e -> loggedIn.setBool(false)).build()
                                        : button("Login").onClick(e -> loggedIn.setBool(true)).build()
{{/AUTH}}
                        )
                ),
                main(
    ...
    static ReactElement renderPage(String page) {
        return switch (page) {
            ...
            default -> section(
                    h1("Welcome to {{APP_NAME_CAMEL}}"),
                    p("Your application is running!")
            );
        };
    }
}
